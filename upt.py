...
class UPT(nn.Module):
    ...
    def associate_with_ground_truth(self, boxes_h, boxes_o, targets):
        ...
        # cls = torch.clamp(torch.sum(labels, dim=0), max=1)
        return labels
    ...
    def forward(self,
        ...
        # src16, _= features[-2].decompose()
        assert mask is not None
        # memory = self.detector.input_proj(src)
        # hs = self.detector.transformer(memory, mask, self.detector.query_embed.weight, pos[-1])[0]
        # memory = torch.cat([memory,memory2],dim=1)
        hs, memory = self.detector.transformer(self.detector.input_proj(src), mask, self.detector.query_embed.weight, pos[-1])
        # memory = self.trans(memory,mask,pos[-1])
        # or_src = self.detector.input_proj(src)
        # memory = memory+or_src
        ...
        #结果放大为原图像大小（经过数据处理）
        #convert to[x0, y0, x1, y1] format and from relative [0, 1] to absolute [0, height] coordinates
        region_props = self.prepare_region_proposals(results, hs[-1])


        # logits, prior, bh, bo, objects, attn_maps, cls_coll= self.interaction_head(
        logits, prior, bh, bo, objects, attn_maps=self.interaction_head(
            features[-1].tensors, image_sizes, region_props, memory, mask#,memory2
        )
        boxes = [r['boxes'] for r in region_props]
        if self.training:
            interaction_loss = self.compute_interaction_loss(boxes, bh, bo, logits, prior, targets)
            # aux_loss = self.l1_loss(box_collated, bh, prebox,targets)
            # cls_loss = self.cls_loss(boxes, bh, bo, cls_coll,prior,targets)
            loss_dict = dict(
                interaction_loss=interaction_loss,
                # aux_loss =0.1*aux_loss
                # cls_loss = 2.0*cls_loss
            )
            return loss_dict

        detections = self.postprocessing(boxes, bh, bo, logits, prior, objects, attn_maps, image_sizes)
        # detections = self.postprocessingg(boxes, bh, bo, logits, prior, objects, attn_maps, image_sizes,logits_s)
        return detections
